  ...
  <div>
    ...
    <ul class="text-muted text-xs mt-4">
      <li class="mb-2" v-for="kv in tags" :key="kv[0]">
        <button @click="tagClicked(kv[0])" class="hover:underline focus:outline-none">{{ kv[0] }}</button> ({{ kv[1] }})
      </li>
    </ul>
  </div>
</template>
...
export default {
  ...
  computed: {
    tags() {
      const unsortedTags = Array.from(Object.entries(this.$store.state.tags))
